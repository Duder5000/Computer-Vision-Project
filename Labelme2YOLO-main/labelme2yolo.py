...
class Labelme2YOLO(object):
    ...
    x_min = max(0, obj_center_x - radius)
    y_min = max(0, obj_center_y - radius)
    x_max = min(img_w, obj_center_x + radius)
    y_max = min(img_h, obj_center_y + radius)

    # Normalize bounding box coordinates
    yolo_center_x = round(float((x_min + x_max) / (2 * img_w)), 6)
    yolo_center_y = round(float((y_min + y_max) / (2 * img_h)), 6)
    yolo_width = round(float((x_max - x_min) / img_w), 6)
